...
function getInlineDeclarationRenderData(declarations, options) {
  const renderDatas = declarations.map((declaration) => {
    ...
    const importPath = options.output != null ? addCurrentDirPrefix(posixRelative(options.output, import_pathe3.default.join(dirname, basename2))) : (0, import_my_node_fp14.replaceSepToPosix)(`.${import_path.default.posix.sep}${import_pathe3.default.join(dirname, basename2)}`);
    return {
      ...declaration,
      importPath,
      extname: {
