...
var _ProgressBar = class _ProgressBar {
  ...
  static bootstrap() {
    ...
    const bar = new import_cli_progress.SingleBar({
      format: `Progress [${import_chalk.default.green("{bar}")}] {percentage}% | {value}/{total}`,
      barCompleteChar: "\u25A0",
      barIncompleteChar: " ",
      stopOnComplete: true,
      barsize: 40,
  ...
  set stream(value) {
    if (value !== __privateGet(this, _stream)) {
      ...
      __privateSet(this, _bar, new import_cli_progress.SingleBar({
        format: `${__privateGet(this, _head)}[${import_chalk.default.green("{bar}")}] {percentage}% | {value}/{total}`,
        barCompleteChar: "\u25A0",
        barIncompleteChar: " ",
        stopOnComplete: true,
        barsize: 40,
...
var _Reasoner = class _Reasoner {
  ...
  static messaging(reason) {
    const messageBlock = [""];
    const typeMessage = reason.type === "error" ? import_chalk2.default.bgRed(`   ${reason.type.toUpperCase()}   `) : import_chalk2.default.bgYellow(`   ${import_chalk2.default.black(reason.type.toUpperCase())}    `);
    const { filePath } = reason;
    const filename = reason.lineAndCharacter == null ? `${path.basename(filePath)}` : `${path.basename(filePath)}:${reason.lineAndCharacter.line}:${reason.lineAndCharacter.character}`;
    const chevronRight = reason.type === "error" ? import_chalk2.default.red(">") : import_chalk2.default.yellow(">");
    messageBlock.push(`${typeMessage} ${filename}`);
    if (reason.lineAndCharacter == null) {
      messageBlock.push(`   ${chevronRight} ${import_chalk2.default.gray(`${filePath}`)}`);
    } else {
      messageBlock.push(
        `   ${chevronRight} ${import_chalk2.default.gray(
    ...
    messageBlock.push(
      ...reason.message.split("\n").map((splittedMessage) => {
        return `   ${chevronRight} ${import_chalk2.default.gray(splittedMessage.trim())}`;
      })
    );
    messageBlock.push("");
...
function getTypeScriptConfig(project) {
  const resolvedProjectPath = import_node_path.default.resolve(project);
  ...
  const tsconfig = tsm.ts.parseJsonConfigFileContent(
    configFile.config,
    parseConfigHost,
    import_node_path.default.dirname(resolvedProjectPath)
  );
  return tsconfig;
}
...
function getOutputValue(argv, option) {
  ...
  return (0, import_my_node_fp.getDirnameSync)(argv.project);
}
...
async function transformCreateMode(argv, option) {
  const startFrom = argv.startFrom ?? option.startFrom ?? import_pathe.default.resolve(await (0, import_my_node_fp2.getDirname)(argv.project));
  const resolvedStartFrom = import_pathe.default.isAbsolute(startFrom) ? startFrom : import_pathe.default.resolve(startFrom);
  return {
...
function getTsIncludeFiles(config) {
  ...
  const filePaths = config.extend.tsconfig.fileNames.filter(
    (filePath) => (0, import_my_node_fp3.isDescendant)(config.extend.resolved.projectDirPath, filePath)
  );
  return filePaths;
}
...
async function createBuildOptions(argv) {
    ...
    options.options = await Promise.all(
      options.options.map(async (option) => {
        if (option.mode === CE_CTIX_BUILD_MODE.MODULE_MODE) {
          const projectPath3 = import_node_path2.default.resolve(option.project);
        ...
        const projectPath2 = import_node_path2.default.resolve(option.project);
  ...
  const projectPath = import_node_path2.default.resolve(argv.project);
  const tsconfig = getTypeScriptConfig(projectPath);
  const include = argv.include != null ? (0, import_my_easy_fp.toArray)(argv.include) : getTsIncludeFiles({
    config: { include: [] },
    extend: { tsconfig, resolved: { projectDirPath: projectPath } }
  });
  const exclude = argv.exclude != null ? (0, import_my_easy_fp.toArray)(argv.exclude) : getTsExcludeFiles({
    config: { exclude: [] },
    extend: { tsconfig }
  });
...
function getJsDocComment(kind, comment) {
  if (kind === tsm2.SyntaxKind.SingleLineCommentTrivia) {
...
function getCommentKind(kind) {
  switch (kind) {
    case tsm3.SyntaxKind.MultiLineCommentTrivia:
      return tsm3.SyntaxKind.MultiLineCommentTrivia;
    default:
      return tsm3.SyntaxKind.SingleLineCommentTrivia;
  }
}
...
function getCommentsWithParent(node) {
  if (node.getKind() === tsm4.SyntaxKind.VariableDeclaration) {
    const parent = node.getParent();
    const grandParent = parent?.getParent();
    const parentComments = parent?.getKind() === tsm4.SyntaxKind.VariableDeclarationList ? parent.getLeadingCommentRanges() : [];
    const grandParentComments = grandParent?.getKind() === tsm4.SyntaxKind.VariableStatement ? grandParent.getLeadingCommentRanges() : [];
...
function getExportAssignmentMap(sourceFile) {
  ...
  const exportAssignmentMap = exportAssignments.map((exportAssignment) => {
    const exportAssignmentName = exportAssignment.getChildren().map((node) => {
      const name = node.getKind() === tsm5.SyntaxKind.Identifier ? node.getText().trim() : void 0;
      return name;
      ...
      return symbols.statements.map((statement) => {
        if (statement.isDefault) {
          const reason2 = {
            ...
            message: `detect same name of default export statement: "${import_chalk3.default.yellow(
        ...
        const reason = {
          ...
          lineAndCharacter: { line: statement.pos.line, character: statement.pos.column },
          filePath: import_pathe2.default.join(statement.path.dirPath, statement.path.filename),
          message: `detect same name of export statement: "${import_chalk3.default.yellow(
            _StatementTable.key(statement)
          )}"`
        };
        return reason;
...
function getFunctionName(kind, name) {
  if (kind === tsm6.SyntaxKind.ArrowFunction) {
    if (name !== "__function" && name != null) {
      return name;
    }
    return void 0;
  }
  if (kind === tsm6.SyntaxKind.FunctionDeclaration) {
    if (name != null) {
      return name;
    }
    return void 0;
...
function getExportedKind(node) {
  ...
  return (0, import_ts_pattern.match)(kind).with(tsm7.SyntaxKind.ClassDeclaration, () => {
    return {
      name: node.asKindOrThrow(tsm7.SyntaxKind.ClassDeclaration).getNameOrThrow().toString(),
      kind: tsm7.SyntaxKind.ClassDeclaration,
      isPureType: false
    };
  }).with(tsm7.SyntaxKind.VariableDeclaration, () => {
    const variableDeclarationNode = node.asKindOrThrow(tsm7.SyntaxKind.VariableDeclaration);
    return {
      name: variableDeclarationNode.getName(),
      kind: tsm7.SyntaxKind.VariableDeclaration,
      isPureType: false
    };
  }).with(tsm7.SyntaxKind.ArrowFunction, () => {
    const arrowFunctionNode = node.asKindOrThrow(tsm7.SyntaxKind.ArrowFunction);
    const name = arrowFunctionNode.getSymbolOrThrow().getEscapedName();
    return {
      name: getFunctionName(tsm7.SyntaxKind.ArrowFunction, name),
      kind: tsm7.SyntaxKind.ArrowFunction,
      isPureType: false
    };
  }).with(tsm7.SyntaxKind.FunctionDeclaration, () => {
    const functionDeclarationNode = node.asKindOrThrow(tsm7.SyntaxKind.FunctionDeclaration);
    const name = functionDeclarationNode.getName();
    return {
      name: getFunctionName(tsm7.SyntaxKind.FunctionDeclaration, name),
      kind: tsm7.SyntaxKind.FunctionDeclaration,
      isPureType: false
    };
  }).with(tsm7.SyntaxKind.InterfaceDeclaration, () => {
    const interfaceDeclarationNode = node.asKindOrThrow(tsm7.SyntaxKind.InterfaceDeclaration);
    const name = interfaceDeclarationNode.getName();
    return {
      name,
      kind: tsm7.SyntaxKind.InterfaceDeclaration,
      isPureType: true
    };
  }).with(tsm7.SyntaxKind.TypeAliasDeclaration, () => {
    const typeAliasDeclarationNode = node.asKindOrThrow(tsm7.SyntaxKind.TypeAliasDeclaration);
    const name = typeAliasDeclarationNode.getName();
    return {
      name,
      kind: tsm7.SyntaxKind.TypeAliasDeclaration,
      isPureType: true
    };
  }).with(tsm7.SyntaxKind.EnumDeclaration, () => {
    const enumDeclarationNode = node.asKindOrThrow(tsm7.SyntaxKind.EnumDeclaration);
    const name = enumDeclarationNode.getName();
    return {
      name,
      kind: tsm7.SyntaxKind.EnumDeclaration,
      isPureType: false
    };
  }).with(tsm7.SyntaxKind.ArrayLiteralExpression, () => {
    return {
      name: void 0,
      kind: tsm7.SyntaxKind.ArrayLiteralExpression,
      isPureType: false
    };
  }).with(tsm7.SyntaxKind.ObjectLiteralExpression, () => {
    return {
      name: void 0,
      kind: tsm7.SyntaxKind.ObjectLiteralExpression,
      isPureType: false
    };
  }).with(tsm7.SyntaxKind.BindingElement, () => {
    const bindingElementNode = node.asKindOrThrow(tsm7.SyntaxKind.BindingElement);
    const name = bindingElementNode.getName();
    return {
      name,
      kind: tsm7.SyntaxKind.BindingElement,
      isPureType: false
    };
  }).with(tsm7.SyntaxKind.CallExpression, () => {
    return {
      name: void 0,
      kind: tsm7.SyntaxKind.CallExpression,
      isPureType: false
    };
  }).with(tsm7.SyntaxKind.NewExpression, () => {
    return {
      name: void 0,
      kind: tsm7.SyntaxKind.NewExpression,
      isPureType: false
    };
  }).with(tsm7.SyntaxKind.ModuleDeclaration, () => {
    const moduleDeclarationNode = node.asKindOrThrow(tsm7.SyntaxKind.ModuleDeclaration);
    const name = moduleDeclarationNode.getName();
    return {
      name,
      kind: tsm7.SyntaxKind.ModuleDeclaration,
      isPureType: false
    };
  }).with(tsm7.SyntaxKind.SourceFile, () => {
    return {
      name: void 0,
      kind: tsm7.SyntaxKind.SourceFile,
      isPureType: false
    };
  }).otherwise(() => {
...
function getExtname(filePath) {
  if (extensions.every((extension) => filePath.endsWith(extension) === false)) {
    return import_node_path3.default.extname(filePath);
  }
...
function filenamify(filename) {
  const basename2 = import_node_path4.default.basename(filename, getExtname(filename));
...
function posixRelative(originPath, targetPath) {
  return (0, import_my_node_fp4.replaceSepToPosix)(path6.relative(originPath, targetPath));
}
...
function getRelativeDepth(basePath, dirPath) {
  ...
  const depth = relativePath.split(import_node_path5.default.posix.sep);
  return depth.length;
}
...
function posixResolve(targetPath) {
  return (0, import_my_node_fp5.replaceSepToPosix)(path8.resolve(targetPath));
}
...
async function getExportStatement(sourceFile, option, extendOptions) {
  const dirPath = posixResolve(await (0, import_my_node_fp6.getDirname)(sourceFile.getFilePath().toString()));
  const filename = (0, import_my_node_fp6.startSepRemove)(
    (0, import_my_node_fp6.replaceSepToPosix)(sourceFile.getFilePath().toString().replace(dirPath, "")),
    import_node_path6.default.posix.sep
  );
  const relativePath = posixRelative(await (0, import_my_node_fp6.getDirname)(option.project), dirPath);
  ...
  const namedExports = Array.from(exportedDeclarationsMap.entries()).filter(([identifier]) => identifier !== "default").map((exportedDeclarationsWithKey) => {
    ...
    if (exportedDeclaration.getKind() === tsm8.SyntaxKind.ModuleDeclaration && kind.name != null) {
    ...
    if (exportedDeclaration.getKind() === tsm8.SyntaxKind.SourceFile) {
      return getSummaryStatement({
        path: { filename, dirPath, relativePath },
        identifier: exportedDeclarationKey,
        node: exportedDeclaration,
...
var declarationKindMap = /* @__PURE__ */ new Map([
  [tsm9.SyntaxKind.ModuleDeclaration, true]
]);
...
function getDepth(dirPath) {
  const sepReplaced = (0, import_my_node_fp7.replaceSepToPosix)(dirPath);
  const depth = sepReplaced.split(import_node_path7.default.posix.sep);
  return depth.length;
}
...
async function getExtendOptions(project) {
  ...
  const tsconfig = getTypeScriptConfig(projectPath);
  const resolvedProjectDirPath = (0, import_my_node_fp8.replaceSepToPosix)(await (0, import_my_node_fp8.getDirname)(projectPath));
  const filePaths = tsconfig.fileNames.filter(
    (filePath) => (0, import_my_node_fp8.isDescendant)(resolvedProjectDirPath, filePath)
  );
  const topDirDepth = (await Promise.all(
    filePaths.map(async (filePath) => {
      const dirPath = (0, import_my_node_fp8.replaceSepToPosix)(posixResolve(await (0, import_my_node_fp8.getDirname)(filePath)));
      return {
        filePaths: [dirPath],
    ...
    (minDepth, depth) => {
      if (minDepth.depth > depth.depth) {
        return { ...depth, filePaths: (0, import_my_easy_fp2.settify)(minDepth.filePaths.concat(depth.filePaths)) };
      }
      if (minDepth.depth === depth.depth) {
        return { ...minDepth, filePaths: (0, import_my_easy_fp2.settify)(minDepth.filePaths.concat(depth.filePaths)) };
      }
      return minDepth;
    },
    {
...
function getTypeScriptProject(projectOption) {
  const project = new tsm10.Project(projectOption);
  return project;
}
...
async function checkOutputFile(outputMap) {
  ...
  const filePathExists = await Promise.all(
    filePaths.map(async (filePath) => {
      return { exists: await (0, import_my_node_fp9.exists)(filePath), filePath };
    })
  );
...
function posixJoin(...args) {
  return (0, import_my_node_fp10.replaceSepToPosix)(args.join(path12.sep));
}
...
function getGlobFiles(glob) {
  ...
  for (const filePath of glob) {
    filePathSet.add(
      typeof filePath === "string" ? (0, import_my_node_fp11.replaceSepToPosix)(filePath) : (0, import_my_node_fp11.replaceSepToPosix)(import_node_path9.default.join(filePath.path, filePath.name))
    );
  }
...
var ExcludeContainer = class {
  constructor(params) {
    ...
    const files = getGlobFiles(globs).map((filePath) => [
      (0, import_my_node_fp12.replaceSepToPosix)(filePath),
      true
    ]);
    ...
    params.inlineExcludeds.forEach((inlineExcluded) => {
      const filePath = import_node_path10.default.isAbsolute(inlineExcluded.filePath) ? (0, import_my_node_fp12.replaceSepToPosix)(inlineExcluded.filePath) : posixResolve(inlineExcluded.filePath);
...
async function indexWrites(indexFiles, option, extendOptions) {
  await Promise.all(
    indexFiles.map(async (file) => {
      ...
      if (option.backup) {
        if (await (0, import_my_node_fp13.exists)(file.path)) {
...
var _TemplateContainer = class _TemplateContainer {
  ...
  static async evaluate(name, data, option) {
    try {
      ...
      const err = (0, import_my_easy_fp3.isError)(caught, new Error("raise error from evaluateTemplate"));
      import_consola.default.error(`template: ${name}`, data);
...
function getInlineDeclarationRenderData(declarations, options) {
  const renderDatas = declarations.map((declaration) => {
    ...
    const dirname = import_pathe3.default.dirname(declaration.filePath);
    const basename2 = import_pathe3.default.basename(declaration.filePath, extname);
    const relativePath = options.output != null ? addCurrentDirPrefix(posixRelative(options.output, import_pathe3.default.join(dirname, basename2))) : (0, import_my_node_fp14.replaceSepToPosix)(`.${import_path.default.posix.sep}${import_pathe3.default.join(dirname, basename2)}`);
    return {
      ...declaration,
      relativePath,
      extname: {
...
function getRenderData(option, filePath, statements, output) {
  ...
  const relativePath = output != null ? addCurrentDirPrefix(posixRelative(output, filename)) : (0, import_my_node_fp15.replaceSepToPosix)(`.${import_node_path13.default.posix.sep}${(0, import_my_node_fp15.basenames)(filename, getExtname(filePath))}`);
  return {
    options: {
      quote: option.quote,
...
async function getOutputExcludedFiles(params) {
  const outputDirPaths = await Promise.all(
    params.filePaths.map((filePath) => params.project.getSourceFile(filePath)).filter((sourceFile) => sourceFile != null).map(async (sourceFile) => {
      ...
      const dirPath = await (0, import_my_node_fp16.getDirname)(filePath);
      return dirPath;
    })
  );
  const outputFiles = (0, import_my_easy_fp4.settify)(outputDirPaths).map(
    (dirPath) => import_pathe4.default.join(dirPath, params.exportFilename)
  );
  return outputFiles;
}
...
function getAllParentDir(parentDir, childDir) {
  const parent = (0, import_my_node_fp17.startSepAppend)(parentDir, import_node_path14.default.posix.sep);
  const child = (0, import_my_node_fp17.startSepAppend)(childDir, import_node_path14.default.posix.sep);
  const elements = (0, import_my_node_fp17.startSepRemove)(child.replace(parent, ""), import_node_path14.default.posix.sep).split(import_node_path14.default.posix.sep).slice(0, -1);
  return [
    parentDir,
    ...(0, import_my_easy_fp5.populate)(elements.length, true).map((index) => {
...
async function dfsWalk(currentDirPath, callback) {
  ...
  const dirPaths = (await Promise.all(
    readed.map(async (filePath) => {
      if (await (0, import_my_node_fp18.isDirectory)(posixJoin(resolved, filePath))) {
        return filePath;
      }
      return void 0;
    })
...
function getParentDir(rawDirPath) {
  const dirPath = (0, import_my_node_fp19.replaceSepToPosix)(rawDirPath);
...
async function creating(_buildOptions, createOption) {
  ...
  Array.from(filePathMap.entries()).map(([filePath, exportStatements]) => getRenderData(createOption, filePath, exportStatements)).filter((renderData) => renderData != null).map((renderData) => {
    ...
    const key = (0, import_my_node_fp20.getDirnameSync)(data.renderData.filePath);
...
async function getModuleRenderData(option, filePath, output) {
  ...
  const dirPath = await (0, import_my_node_fp21.getDirname)(filePath);
  const relativePath = addCurrentDirPrefix(
    posixRelative(await (0, import_my_node_fp21.getDirname)(output), await (0, import_my_node_fp21.getDirname)(filePath))
  );
  const defaultExport = {
    path: {
      filename,
...
async function askInitOptions() {
  ...
  const optionFilePath = import_pathe5.default.join(cwdAnswer.cwd, CE_CTIX_DEFAULT_VALUE.CONFIG_FILENAME);
  ...
  if (userSelectedAnswer.configPosition === ".ctirc" && optionFileExist) {
    ...
    if (!overwriteAnswer.overwirte) {
      return {
        ...
        packageJson: import_pathe5.default.join(process.cwd(), CE_CTIX_DEFAULT_VALUE.PACKAGE_JSON_FILENAME),
        mode: CE_CTIX_BUILD_MODE.BUNDLE_MODE,
...
async function getDefaultInitAnswer() {
  ...
  const answer = {
    ...
    packageJson: import_pathe6.default.join(process.cwd(), CE_CTIX_DEFAULT_VALUE.PACKAGE_JSON_FILENAME),
    mode: CE_CTIX_BUILD_MODE.BUNDLE_MODE,
    exportFilename: CE_CTIX_DEFAULT_VALUE.EXPORT_FILENAME,
    addEveryOptions: false,
    configComment: true,
...
async function initializing(option) {
  ...
  if (!answer.overwirte) {
    const optionFilePath = import_pathe7.default.join(answer.cwd, CE_CTIX_DEFAULT_VALUE.CONFIG_FILENAME);
  ...
  if (answer.configPosition === "tsconfig.json") {
    await Promise.all(
      answer.tsconfig.map(async (tsconfigFilePath) => {
        const resolvedTsconfigFilePath = import_pathe7.default.resolve(tsconfigFilePath);
        ...
        if (answer.confirmBackupPackageTsconfig) {
          const backupFilePath = `${import_pathe7.default.basename(resolvedTsconfigFilePath)}.bak${import_pathe7.default.extname(resolvedTsconfigFilePath)}`;
    ...
    const packageJsonFilePath = import_pathe7.default.resolve(answer.packageJson);
    ...
    if (answer.confirmBackupPackageTsconfig) {
      const backupFilePath = `${import_pathe7.default.basename(packageJsonFilePath)}.bak${import_pathe7.default.extname(packageJsonFilePath)}`;
      await import_node_fs4.default.promises.writeFile(backupFilePath, buf.toString());
    }
...
async function getRemoveFileGlobPattern(argv, options) {
  const dirs = (await Promise.all(
    options.map(async (modeOption) => {
      const dir = await (0, import_my_node_fp22.getDirname)(modeOption.project);
      const results = [
        {
          origin: dir,
...
async function unlinks(filePaths, callback) {
  await Promise.all(
    filePaths.map(async (filePath) => {
      if (await (0, import_my_node_fp23.exists)(filePath)) {
...
function getConfigValue(argv, ...keywordArgs) {
  ...
  const key = (0, import_my_easy_fp6.atOrUndefined)(keys, 0);
...
function getCommand(raw) {
  const command = (0, import_my_easy_fp7.atOrUndefined)((0, import_my_easy_fp7.toArray)(raw), 0);
  switch (command) {
    case CE_CTIX_COMMAND.BUILD_COMMAND:
...
async function getConfigFilePath(fileName, configFilePath) {
  ...
  const cwdConfigFilePath = import_pathe8.default.join(process.cwd(), fileName);
  if (await (0, import_my_node_fp24.exists)(cwdConfigFilePath)) {
...
function readJson5(buf) {
  try {
    ...
    const err = (0, import_my_easy_fp8.isError)(caught, new Error("unknown error raised"));
...
function readJsonc(buf) {
  try {
    ...
    if (errors.length > 0) {
      throw new Error(
        `JSONC: [${(0, import_jsonc_parser.printParseErrorCode)((0, import_my_easy_fp9.atOrThrow)(errors, 0).error)}] invalid character ${(0, import_my_easy_fp9.atOrThrow)(errors, 0).length}:${(0, import_my_easy_fp9.atOrThrow)(errors, 0).offset}`
      );
    }
    return (0, import_my_only_either2.pass)(parsed);
  } catch (caught) {
    const err = (0, import_my_easy_fp9.isError)(caught, new Error("unknown error raised"));
...
function readYaml(buf) {
  try {
    ...
    const err = (0, import_my_easy_fp10.isError)(caught, new Error("unknown error raised"));
...
async function readConfigFromFile(configFilePath) {
  try {
    ...
    const err = (0, import_my_easy_fp11.isError)(caught, new Error("unknown error raised from configuration reading"));
...
async function readConfigFromPackageJson() {
  try {
    const packageJsonFilePath = import_pathe9.default.join(process.cwd(), "package.json");
    ...
    const err = (0, import_my_easy_fp12.isError)(caught, new Error("unknown error raised from configuration reading"));
...
async function readConfigFromTsconfigJson(tsconfigFilePath) {
  try {
    ...
    const err = (0, import_my_easy_fp13.isError)(caught, new Error("unknown error raised from configuration reading"));
...
async function loadConfig() {
  try {
    ...
    const err = (0, import_my_easy_fp14.isError)(catched, new Error("unknown error raised"));
    import_consola5.default.debug(err);
    return {};
  }
}

// src/cli.ts
var import_consola6 = __toESM(require("consola"));
...
var buildCommandModule = {
  ...
  builder: (argv) => {
    const projectArgv = setProjectOptions(
      argv
    );
    const generateArgv = setModeGenerateOptions(projectArgv);
    const bundleArgv = setModeBundleOptions(generateArgv);
    const createArgv = setModeCreateOptions(bundleArgv);
    return createArgv;
  },
  handler: async (argv) => {
    try {
      ...
      const err = (0, import_my_easy_fp15.isError)(catched, new Error("unknown error raised from bundle command"));
      import_consola6.default.error(err);
    }
  }
};
var removeCommandModule = {
  command: CE_CTIX_COMMAND.REMOVE_COMMAND,
  aliases: [CE_CTIX_COMMAND.REMOVE_COMMAND_ALIAS],
  describe: "remove index.ts file",
  builder: (argv) => {
    const commonArgv = setProjectOptions(
      argv
    );
    const removeArgv = setCommandRemoveOptions(commonArgv);
    return removeArgv;
  },
  handler: async (argv) => {
    try {
      ...
      const err = (0, import_my_easy_fp15.isError)(catched, new Error("unknown error raised from remove command"));
      import_consola6.default.error(err);
    }
  }
};
var initCommandModule = {
  command: CE_CTIX_COMMAND.INIT_COMMAND,
  aliases: [CE_CTIX_COMMAND.INIT_COMMAND_ALIAS],
  describe: "create .ctirc configuration",
  builder: (argv) => {
    const initArgv = setCommandInitOptions(
      argv
    );
    return initArgv;
  },
  handler: async (argv) => {
    try {
      ...
      const err = (0, import_my_easy_fp15.isError)(catched, new Error("unknown error raised from init command"));
      import_consola6.default.error(err);
    }
  }
};
var handler = async () => {
  const parser = (0, import_yargs.default)((0, import_helpers.hideBin)(process.argv));
  parser.command(buildCommandModule).command(removeCommandModule).command(initCommandModule).demandCommand().recommendCommands().config(await loadConfig()).help();
  await parser.argv;
};
handler().catch((caught) => {
  const err = (0, import_my_easy_fp15.isError)(caught, new Error("unknown error raised"));
  import_consola6.default.error(err);
  process.exit(1);
});
