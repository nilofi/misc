...
function getInlineCommented(params) {
    ...
    const tag = block.tags.find(
        element => element.tag === getJsDocTag(params.options.keyword),
    );
    if (
        tag?.tag === params.options.keyword ||
        tag?.tag === params.options.keyword.substring(1)
    ) {
        return {
            commentCode: content,
            ...
            tag: tag.tag,
            workspaces: getCommentWorkspaces(
    ...
    const exportAssignmentMap = exportAssignments
        .map(exportAssignment => {
            const exportAssignmentName = exportAssignment
                ...
                .map(node => {
                    const name =
                        node.getKind() === tsm4.SyntaxKind.Identifier
                            ? node.getText().trim()
                            : void 0;
                    return name;
                })
            ...
            return {
                ...
                name: exportAssignmentName ?? "__default",
            };
        })
...
function getNodeComments(node, identifier) {
    ...
    const exportAssignment =
        identifier != null ? exportAssignmentMap.get(identifier) : void 0;
...
import { parse as parse2 } from "comment-parser";
function getInlineStyle(params) {
    const content = params.comment.range;
    const refined = getJsDocComment(params.comment.kind, content);
    const blocks = parse2(refined);
    const block = blocks.at(0);
    if (block == null) {
        return void 0;
    }
...
import * as tsm6 from "ts-morph";
import { match } from "ts-pattern";
function getExportedKind(node) {
    const kind = node.getKind();
    return match(kind)
        .with(tsm6.SyntaxKind.ClassDeclaration, () => {
            return {
                name: node
                    .asKindOrThrow(tsm6.SyntaxKind.ClassDeclaration)
                    .getNameOrThrow()
                    .toString(),
        ...
        .with(tsm6.SyntaxKind.VariableDeclaration, () => {
            const variableDeclarationNode = node.asKindOrThrow(
                tsm6.SyntaxKind.VariableDeclaration,
            );
            return {
        ...
        .with(tsm6.SyntaxKind.ArrowFunction, () => {
            const arrowFunctionNode = node.asKindOrThrow(
                tsm6.SyntaxKind.ArrowFunction,
            );
        ...
        .with(tsm6.SyntaxKind.FunctionDeclaration, () => {
            const functionDeclarationNode = node.asKindOrThrow(
                tsm6.SyntaxKind.FunctionDeclaration,
            );
        ...
        .with(tsm6.SyntaxKind.InterfaceDeclaration, () => {
            const interfaceDeclarationNode = node.asKindOrThrow(
                tsm6.SyntaxKind.InterfaceDeclaration,
            );
        ...
        .with(tsm6.SyntaxKind.TypeAliasDeclaration, () => {
            const typeAliasDeclarationNode = node.asKindOrThrow(
                tsm6.SyntaxKind.TypeAliasDeclaration,
            );
        ...
        .with(tsm6.SyntaxKind.EnumDeclaration, () => {
            const enumDeclarationNode = node.asKindOrThrow(
                tsm6.SyntaxKind.EnumDeclaration,
            );
        ...
        .with(tsm6.SyntaxKind.BindingElement, () => {
            const bindingElementNode = node.asKindOrThrow(
                tsm6.SyntaxKind.BindingElement,
            );
        ...
        .with(tsm6.SyntaxKind.ModuleDeclaration, () => {
            const moduleDeclarationNode = node.asKindOrThrow(
                tsm6.SyntaxKind.ModuleDeclaration,
            );
...
function getStatementAlias({ alias, isDefault, filenamified, kind }) {
...
function filenamify(filename) {
    const basename2 = path2.basename(filename, getExtname(filename));
    if (/^([A-Z])(.+)/.test(basename2)) {
        const camel = camelCase(basename2);
        const upperFirstCase = camel.charAt(0).toUpperCase() + camel.slice(1);
        return doFilenamify(upperFirstCase);
    }
    const raw = camelCase(basename2);
...
function posixRelative(originPath, targetPath) {
    return replaceSepToPosix(path3.relative(originPath, targetPath));
}
...
function getRelativeDepth(basePath, dirPath) {
    ...
    const depth = relativePath.split(path4.posix.sep);
    return depth.length;
}
    ...
    const comments = getNodeComments(params.node, params.identifier)
        .map(comment =>
            getInlineCommented({
                ...
                options: {
                    keyword:
                        CE_INLINE_COMMENT_KEYWORD.NEXT_STATEMENT_EXCLUDE_KEYWORD,
                },
            }),
        )
        .filter(comment => comment != null);
    const pos = params.node
        .getSourceFile()
        .getLineAndColumnAtPos(params.node.getStart(false));
    return {
        path: params.path,
...
import { replaceSepToPosix as replaceSepToPosix2 } from "my-node-fp";
import * as path5 from "path";
function posixResolve(targetPath) {
    return replaceSepToPosix2(path5.resolve(targetPath));
}

// src/compilers/getExportStatement.ts
import {
    getDirname as getDirname2,
    replaceSepToPosix as replaceSepToPosix3,
    startSepRemove,
} from "my-node-fp";
import path6 from "path";
import * as tsm7 from "ts-morph";
async function getExportStatement(sourceFile, option, extendOptions) {
    const dirPath = posixResolve(
        await getDirname2(sourceFile.getFilePath().toString()),
    );
    ...
    const relativePath = posixRelative(
        await getDirname2(option.project),
        dirPath,
    );
    const exportedDeclarationsMap = sourceFile.getExportedDeclarations();
    const defaultExportedDeclarations = exportedDeclarationsMap
        .get("default")
        ?.at(0);
    ...
    const namedExports = Array.from(exportedDeclarationsMap.entries())
        ...
        .map(exportedDeclarationsWithKey => {
            const [exportedDeclarationKey, exportedDeclarations] =
                exportedDeclarationsWithKey;
            const [exportedDeclaration] = exportedDeclarations;
            const kind = getExportedKind(exportedDeclaration);
            if (
                exportedDeclaration.getKind() ===
                    tsm7.SyntaxKind.ModuleDeclaration &&
                kind.name != null
            ) {
            ...
            return getSummaryStatement({
                ...
                identifier: exportedDeclarationKey,
                alias:
                    kind.name !== exportedDeclarationKey && kind.name != null
                        ? kind.name
                        : void 0,
                project: option.project,
                eol: extendOptions.eol,
...
import * as tsm8 from "ts-morph";
function getTypeScriptConfig(project) {
    const resolvedProjectPath = path7.resolve(project);
    ...
    const configFile = tsm8.ts.readConfigFile(
        resolvedProjectPath,
        tsm8.ts.sys.readFile.bind(tsm8.ts),
    );
    const tsconfig = tsm8.ts.parseJsonConfigFileContent(
        configFile.config,
        parseConfigHost,
        path7.dirname(resolvedProjectPath),
    );
    return tsconfig;
}

// src/compilers/getTypeScriptProject.ts
import * as tsm9 from "ts-morph";
...
var _StatementTable = class _StatementTable {
    ...
    static key(statement) {
        ...
        const tableKey = statement.isDefault
    ...
    isDuplicate(statement) {
        const prev = __privateGet(this, _table).get(
    ...
    isDuplicateFromSecond(statement) {
        ...
        const prevStatementKey = `${pathe2.join(first.path.dirPath, first.path.filename)}::${first.identifier.alias}`;
        const nextStatementKey = `${pathe2.join(statement.path.dirPath, statement.path.filename)}::${statement.identifier.alias}`;
    ...
    getDuplicateReason() {
        const reasons = Array.from(__privateGet(this, _table).entries())
                    ...
                    if (statement.isDefault) {
                        const reason2 = {
                            ...
                            lineAndCharacter: {
                                ...
                                character: statement.pos.column,
                            },
                            filePath: pathe2.join(
...
function getConfigObject(argv, ...keywordArgs) {
    ...
    const keys = keywords.filter(
        keyword => keyword in argv && argv[keyword] != null,
    );
    if (keys.length <= 0) {
        return void 0;
    }
...
function getConfigValue(argv, ...keywordArgs) {
    ...
    const keys = keywords.find(
        keyword => keyword in argv && typeof argv[keyword] === "string",
    );
...
import { replaceSepToPosix as replaceSepToPosix4 } from "my-node-fp";
import path8 from "path";
function getDepth(dirPath) {
    const sepReplaced = replaceSepToPosix4(dirPath);
    const depth = sepReplaced.split(path8.posix.sep);
    return depth.length;
}

// src/configs/getExtendOptions.ts
import { settify as settify2 } from "my-easy-fp";
import {
    getDirname as getDirname3,
    isDescendant,
    replaceSepToPosix as replaceSepToPosix5,
} from "my-node-fp";
async function getExtendOptions(project) {
    ...
    const resolvedProjectDirPath = replaceSepToPosix5(
        await getDirname3(projectPath),
    );
    ...
    const topDirDepth = (
        await Promise.all(
            filePaths.map(async filePath => {
                const dirPath = replaceSepToPosix5(
                    posixResolve(await getDirname3(filePath)),
                );
                return {
                    filePaths: [dirPath],
        ...
        (minDepth, depth) => {
            if (minDepth.depth > depth.depth) {
                return {
                    ...depth,
                    filePaths: settify2(
                        minDepth.filePaths.concat(depth.filePaths),
                    ),
                };
            }
            if (minDepth.depth === depth.depth) {
...
import { atOrUndefined as atOrUndefined2, toArray } from "my-easy-fp";
function getCommand(raw) {
    const command = atOrUndefined2(toArray(raw), 0);
    switch (command) {
        case CE_CTIX_COMMAND.BUILD_COMMAND:
...
async function getConfigFilePath(fileName, configFilePath) {
    ...
    const cwdConfigFilePath = pathe3.join(process.cwd(), fileName);
...
const { parse: parse3 } = pkg;
function readJson5(buf) {
    try {
        const stringified = getString(buf);
        const parsed = parse3(stringified);
        return pass(parsed);
    } catch (caught) {
...
import { parse as parse4, printParseErrorCode } from "jsonc-parser";
import { atOrThrow, isError as isError2 } from "my-easy-fp";
import { fail as fail2, pass as pass2 } from "my-only-either";
function readJsonc(buf) {
    try {
        const stringified = getString(buf);
        const errors = [];
        const parsed = parse4(stringified, errors);
        if (errors.length > 0) {
            throw new Error(
                `JSONC: [${printParseErrorCode(atOrThrow(errors, 0).error)}] invalid character ${atOrThrow(errors, 0).length}:${atOrThrow(errors, 0).offset}`,
            );
        }
        return pass2(parsed);
    } catch (caught) {
        const err = isError2(caught, new Error("unknown error raised"));
        return fail2(err);
    }
}
...
import { fail as fail3, pass as pass3 } from "my-only-either";
import { parse as parse5 } from "yaml";
function readYaml(buf) {
    try {
        const stringified = getString(buf);
        const parsed = parse5(stringified);
        return pass3(parsed);
    } catch (caught) {
        const err = isError3(caught, new Error("unknown error raised"));
        return fail3(err);
    }
}
...
import { fail as fail4, pass as pass4 } from "my-only-either";
async function readConfigFromFile(configFilePath) {
    try {
        const buf = await fs2.promises.readFile(configFilePath);
        const parsed = parseConfig(buf);
        if (typeof parsed !== "object") {
            return fail4(
                new Error(`invalid configuration file format: ${parsed}`),
            );
        }
        return pass4(parsed);
    } catch (caught) {
        const err = isError4(
            caught,
            new Error("unknown error raised from configuration reading"),
        );
        return fail4(err);
    }
}
...
import { fail as fail5, pass as pass5 } from "my-only-either";
import pathe4 from "pathe";
async function readConfigFromPackageJson() {
    try {
        const packageJsonFilePath = pathe4.join(process.cwd(), "package.json");
        const buf = await fs3.promises.readFile(packageJsonFilePath);
        ...
        return fail5(new Error("cannot read configuration from package.json"));
    } catch (caught) {
        const err = isError5(
            caught,
            new Error("unknown error raised from configuration reading"),
...
import fs4 from "fs";
import { isError as isError6 } from "my-easy-fp";
import { fail as fail6, pass as pass6 } from "my-only-either";
async function readConfigFromTsconfigJson(tsconfigFilePath) {
    try {
        const buf = await fs4.promises.readFile(tsconfigFilePath);
        ...
        return fail6(
            new Error(`cannot read configuration from ${tsconfigFilePath}`),
        );
    } catch (caught) {
        const err = isError6(
            caught,
...
import { isError as isError7 } from "my-easy-fp";
async function loadConfig() {
    try {
        ...
        const configFileEither =
            configFilePath != null
                ? await readConfigFromFile(configFilePath)
                : void 0;
        if (configFileEither != null && configFileEither.type === "pass") {
            const projectFilePath =
                ...
                getCheckedValue("String", configFileEither.pass.p) ??
                getCheckedValue("String", configFileEither.pass.project) ??
                tsconfigFilePath;
        ...
        const tsconfigEither =
            tsconfigFilePath != null
                ? await readConfigFromTsconfigJson(tsconfigFilePath)
                : void 0;
        ...
        const err = isError7(catched, new Error("unknown error raised"));
        consola.debug(err);
        return {};
    }
}
...
var _ProgressBar = class _ProgressBar {
    ...
    static bootstrap() {
        ...
        const bar = new SingleBar({
            format: `Progress [${chalk2.green("{bar}")}] {percentage}% | {value}/{total}`,
            barCompleteChar: "\u25A0",
            barIncompleteChar: " ",
            stopOnComplete: true,
            barsize: 40,
        ...
        __privateSet(
            _ProgressBar,
            _it,
            new _ProgressBar(bar, stream, isEnable),
        );
    ...
    set stream(value) {
        if (value !== __privateGet(this, _stream)) {
            ...
            __privateSet(
                ...
                _bar,
                new SingleBar({
                    format: `${__privateGet(this, _head)}[${chalk2.green("{bar}")}] {percentage}% | {value}/{total}`,
                    barCompleteChar: "\u25A0",
                    barIncompleteChar: " ",
                    stopOnComplete: true,
                    barsize: 40,
...
var _Reasoner = class _Reasoner {
    ...
    static bootstrap() {
        ...
        __privateSet(
            _Reasoner,
            _it2,
            new _Reasoner(console.error, stream, enable),
        );
    ...
    set stream(value) {
        if (value !== __privateGet(this, _stream2)) {
            __privateSet(
                this,
                _streamFunc,
                value === "stderr" ? console.error : console.log,
            );
    ...
    static messaging(reason) {
        ...
        const typeMessage =
            reason.type === "error"
                ? chalk3.bgRed(`   ${reason.type.toUpperCase()}   `)
                : chalk3.bgYellow(
                      `   ${chalk3.black(reason.type.toUpperCase())}    `,
                  );
        const { filePath } = reason;
        const filename =
            reason.lineAndCharacter == null
                ? `${path9.basename(filePath)}`
                : `${path9.basename(filePath)}:${reason.lineAndCharacter.line}:${reason.lineAndCharacter.character}`;
        const chevronRight =
            reason.type === "error" ? chalk3.red(">") : chalk3.yellow(">");
        messageBlock.push(`${typeMessage} ${filename}`);
        if (reason.lineAndCharacter == null) {
            messageBlock.push(
                `   ${chevronRight} ${chalk3.gray(`${filePath}`)}`,
            );
        } else {
            messageBlock.push(
                `   ${chevronRight} ${chalk3.gray(
        ...
        messageBlock.push(
            ...reason.message.split("\n").map(splittedMessage => {
                return `   ${chevronRight} ${chalk3.gray(splittedMessage.trim())}`;
            }),
        );
        messageBlock.push("");
    ...
    start(reasons) {
        ...
        const errors = reasons
            ...
            .map(reason => _Reasoner.messaging(reason));
        const warns = reasons
    ...
    set stream(value) {
        if (value !== __privateGet(this, _stream3)) {
            ...
            __privateSet(
                ...
                _spinner,
                ora({
                    text: __privateGet(this, _spinner).text,
                    stream: process[value],
                }),
            );
...
function transformBundleMode(argv, option) {
    ...
    return {
        ...
        exportFilename:
            ...
            CE_CTIX_DEFAULT_VALUE.EXPORT_FILENAME,
        ...
        generationStyle:
            argv.generationStyle ??
            option.generationStyle ??
            CE_GENERATION_STYLE.AUTO,
...
async function transformCreateMode(argv, option) {
    const startFrom =
        argv.startFrom ??
        option.startFrom ??
        pathe5.resolve(await getDirname4(argv.project));
    const resolvedStartFrom = pathe5.isAbsolute(startFrom)
        ? startFrom
        : pathe5.resolve(startFrom);
    return {
...
import { isDescendant as isDescendant2 } from "my-node-fp";
function getTsIncludeFiles(config) {
    ...
    const filePaths = config.extend.tsconfig.fileNames.filter(filePath =>
        isDescendant2(config.extend.resolved.projectDirPath, filePath),
    );
    return filePaths;
}

// src/configs/transforms/createBuildOptions.ts
import { toArray as toArray2 } from "my-easy-fp";
...
async function createBuildOptions(argv) {
        ...
        options.options = await Promise.all(
            options.options.map(async option => {
                if (option.mode === CE_CTIX_BUILD_MODE.MODULE_MODE) {
                    const projectPath3 = path10.resolve(option.project);
                    ...
                    const moduleMode = await transformModuleMode(
                        { ...argv, project: projectPath3 },
                        {
                            ...
                            include: getTsIncludeFiles({
                                ...
                                extend: {
                                    tsconfig: tsconfig3,
                                    resolved: { projectDirPath: projectPath3 },
                                },
                            }),
                ...
                const projectPath2 = path10.resolve(option.project);
                ...
                const bundleMode = transformBundleMode(
                    { ...argv, project: projectPath2 },
                    {
                        ...
                        include: getTsIncludeFiles({
                            ...
                            extend: {
                                tsconfig: tsconfig2,
                                resolved: { projectDirPath: projectPath2 },
                            },
                        }),
                        exclude: getTsExcludeFiles({
                            config: { exclude: option.exclude },
    ...
    const projectPath = path10.resolve(argv.project);
    const tsconfig = getTypeScriptConfig(projectPath);
    const include =
        argv.include != null
            ? toArray2(argv.include)
            : getTsIncludeFiles({
                  config: { include: [] },
                  extend: {
                      tsconfig,
                      resolved: { projectDirPath: projectPath },
                  },
              });
    const exclude =
        argv.exclude != null
            ? toArray2(argv.exclude)
            : getTsExcludeFiles({
                  config: { exclude: [] },
                  extend: { tsconfig },
              });
...
var _ProjectContainer = class _ProjectContainer {
    ...
    static project(projectPath) {
        if (!__privateGet(_ProjectContainer, _isBootstrap4)) {
            throw new Error(
                "NOT_INITIALIZE_ERROR: please, initialize before use",
            );
        }
    ...
    static addSourceFilesAtPaths(projectPath, filePaths) {
        ...
        return __privateGet(_ProjectContainer, _it4).addSourceFilesAtPaths(
            projectPath,
            filePaths,
        );
    }
    ...
    project(projectPath) {
        ...
        const loadedProject = getTypeScriptProject({
            tsConfigFilePath: path11.resolve(projectPath),
        });
...
import { replaceSepToPosix as replaceSepToPosix6 } from "my-node-fp";
import * as path12 from "path";
function posixJoin(...args) {
    return replaceSepToPosix6(args.join(path12.sep));
}

// src/modules/file/getGlobFiles.ts
import { replaceSepToPosix as replaceSepToPosix7 } from "my-node-fp";
    ...
    for (const filePath of glob) {
        filePathSet.add(
            typeof filePath === "string"
                ? replaceSepToPosix7(filePath)
                : replaceSepToPosix7(path13.join(filePath.path, filePath.name)),
        );
    }
...
import { replaceSepToPosix as replaceSepToPosix8 } from "my-node-fp";
...
var ExcludeContainer = class {
    constructor(params) {
        ...
        params.inlineExcludeds.forEach(inlineExcluded => {
            const filePath = path14.isAbsolute(inlineExcluded.filePath)
                ? replaceSepToPosix8(inlineExcluded.filePath)
    ...
    isExclude(filePath) {
        if (
            __privateGet(this, _map).size <= 0 &&
            __privateGet(this, _inline).size <= 0
        ) {
            return false;
        }
        if (path14.isAbsolute(filePath)) {
            return (
                __privateGet(this, _map).get(filePath) != null ||
                __privateGet(this, _inline).get(filePath) != null
            );
        }
        return (
            __privateGet(this, _map).get(posixResolve(filePath)) != null ||
            __privateGet(this, _inline).get(posixResolve(filePath)) != null
        );
    }
};
_globs = new WeakMap();
...
var IncludeContainer = class {
    constructor(params) {
        ...
        const globs = new Glob2(params.config.include, {
            absolute: true,
            ignore: defaultExclude,
            cwd: params.cwd,
            windowsPathsNoEscape: true,
    ...
    await Promise.all(
        indexFiles.map(async file => {
            ...
            if (option.backup) {
                ...
                await writeFile(
                    file.path,
                    `${prettified.contents.trim()}${extendOptions.eol}`,
                );
            } else {
                await writeFile(
                    file.path,
...
import { Eta } from "eta";
import fs5 from "fs";
import { isError as isError8 } from "my-easy-fp";
...
var _TemplateContainer = class _TemplateContainer {
    constructor(args) {
        ...
        __privateSet(
            this,
            _eta,
            new Eta({ views: "ctix", autoEscape: false, rmWhitespace: true }),
        );
    ...
    static getDefaultTemplate() {
        return /* @__PURE__ */ new Map([
            [
                ...
                declarationFileTemplate.trim(),
            ],
            [
                CE_TEMPLATE_NAME.MODULE_INDEX_FILE_TEMPLATE,
                moduleIndexFileDefaultTemplate.trim(),
            ],
            [
                CE_TEMPLATE_NAME.INDEX_FILE_TEMPLATE,
                indexFileDefaultTemplate.trim(),
            ],
            [CE_TEMPLATE_NAME.OPTIONS_TEMPLATE, optionDefaultTemplate.trim()],
            [
                CE_TEMPLATE_NAME.NESTED_OPTIONS_TEMPLATE,
                nestedOptionDefaultTemplate.trim(),
            ],
            [
                CE_GENERATION_STYLE.DEFAULT_ALIAS_NAMED_STAR,
                defaultAliasNamedStarDefaultTemplate.trim(),
            ],
            [
                CE_GENERATION_STYLE.DEFAULT_ALIAS_NAMED_DESTRUCTIVE,
            ...
            [
                CE_GENERATION_STYLE.DEFAULT_STAR_NAMED_STAR,
                defaultStarNamedStarDefaultTemplate.trim(),
            ],
            [
                CE_GENERATION_STYLE.DEFAULT_STAR_NAMED_DESTRUCTIVE,
    ...
    static async load(templatePath) {
        ...
        const templates = await _TemplateContainer.readFiles(
        ...
        return /* @__PURE__ */ new Map([
            ...
            [
                CE_TEMPLATE_NAME.MODULE_INDEX_FILE_TEMPLATE,
                templates.moduleIndexFile,
            ],
            [
                CE_TEMPLATE_NAME.DECLARATION_FILE_TEMPLATE,
                templates.declarationFile,
            ],
            [
                CE_GENERATION_STYLE.DEFAULT_ALIAS_NAMED_DESTRUCTIVE,
                templates.defaultAliasNamedDestructive,
            ],
            [
                CE_GENERATION_STYLE.DEFAULT_NON_ALIAS_NAMED_DESTRUCTIVE,
            ...
            [
                CE_GENERATION_STYLE.DEFAULT_STAR_NAMED_STAR,
                templates.defaultStarNamedStar,
            ],
            [
                CE_GENERATION_STYLE.DEFAULT_ALIAS_NAMED_STAR,
                templates.defaultAliasNamedStar,
            ],
            [
                CE_GENERATION_STYLE.DEFAULT_STAR_NAMED_DESTRUCTIVE,
                templates.defaultStarNamedDestructive,
            ],
        ]);
    }
    static async bootstrap(templatePath) {
        ...
        __privateSet(
            _TemplateContainer,
            _it5,
            new _TemplateContainer({ templatePath, templates }),
        );
    ...
    static async readFiles(basePath) {
        ...
        const buffers = await Promise.all([
            fs5.promises.readFile(n(CE_TEMPLATE_NAME.INDEX_FILE_TEMPLATE)),
            fs5.promises.readFile(n(CE_TEMPLATE_NAME.OPTIONS_TEMPLATE)),
            fs5.promises.readFile(n(CE_TEMPLATE_NAME.NESTED_OPTIONS_TEMPLATE)),
            fs5.promises.readFile(
                n(CE_TEMPLATE_NAME.MODULE_INDEX_FILE_TEMPLATE),
            ),
            fs5.promises.readFile(
                n(CE_TEMPLATE_NAME.DECLARATION_FILE_TEMPLATE),
            ),
            fs5.promises.readFile(
                n(CE_GENERATION_STYLE.DEFAULT_ALIAS_NAMED_STAR),
            ),
            fs5.promises.readFile(
                n(CE_GENERATION_STYLE.DEFAULT_ALIAS_NAMED_DESTRUCTIVE),
            ),
            fs5.promises.readFile(
                n(CE_GENERATION_STYLE.DEFAULT_NON_ALIAS_NAMED_DESTRUCTIVE),
            ),
            fs5.promises.readFile(
                n(CE_GENERATION_STYLE.DEFAULT_STAR_NAMED_STAR),
            ),
            fs5.promises.readFile(
                n(CE_GENERATION_STYLE.DEFAULT_STAR_NAMED_DESTRUCTIVE),
            ),
        ]);
        const [
            indexFile,
    ...
    static async evaluate(name, data, option) {
        try {
            ...
            const rendered = await _TemplateContainer.it.evaluate(
                name,
                data,
                option,
            );
            ...
            const err = isError8(
                caught,
                new Error("raise error from evaluateTemplate"),
            );
            consola2.error(`template: ${name}`, data);
            consola2.error(err);
            throw err;
        }
    }
    ...
    etaReadFile(templatePath) {
        return (
            __privateGet(this, _templates).get(templatePath) ??
            defaultStarNamedStarDefaultTemplate.trim()
        );
    }
    async evaluate(name, data, option) {
        try {
            ...
            if (option != null) {
                const rendered2 = __privateGet(this, _eta)
                    .withConfig(option)
                    .render(name, data);
                return rendered2;
            }
...
function addCurrentDirPrefix(filePath, sep2) {
    const pathSep = sep2 ?? path16.posix.sep;
...
import { replaceSepToPosix as replaceSepToPosix9 } from "my-node-fp";
...
function getInlineDeclarationRenderData(declarations, options) {
    const renderDatas = declarations.map(declaration => {
        ...
        const renderExtname = getImportStatementExtname(
            options.fileExt,
            extname,
        );
        const dirname = pathe6.dirname(declaration.filePath);
        const basename2 = pathe6.basename(declaration.filePath, extname);
        const relativePath =
            options.output != null
                ? addCurrentDirPrefix(
                      posixRelative(
                          options.output,
                          pathe6.join(dirname, basename2),
                      ),
                  )
                : replaceSepToPosix9(
                      `.${path17.posix.sep}${pathe6.join(dirname, basename2)}`,
                  );
        return {
            ...declaration,
            relativePath,
...
import {
    basenames,
    replaceSepToPosix as replaceSepToPosix10,
} from "my-node-fp";
...
function getRenderData(option, filePath, statements, output) {
    ...
    const defaultExport = included.find(
        statement => !statement.isExcluded && statement.isDefault,
    );
    ...
    const relativePath =
        output != null
            ...
            : replaceSepToPosix10(
                  `.${path18.posix.sep}${basenames(filename, getExtname(filePath))}`,
              );
    return {
        options: {
            quote: option.quote,
...
function getSelectStyle(params) {
    if (params.comment != null) {
        const style = getInlineStyle({
            ...
            options: {
                keyword:
                    CE_INLINE_COMMENT_KEYWORD.FILE_GENERATION_STYLE_KEYWORD,
            },
        });
        if (style != null) {
            return {
...
async function bundling(buildOptions, bundleOption) {
    ...
    if (
        ...
        Spinner.it.succeed(
            `ctix 'bundle' mode configuration reading from '${buildOptions.from}'`,
        );
    }
    ...
    const output = posixResolve(
        posixJoin(bundleOption.output, bundleOption.exportFilename),
    );
    ...
    Spinner.it.start(
        `build ${`"${chalk4.green(bundleOption.exportFilename)}"`} file start`,
    );
    statements
    ...
    const datas = Array.from(statementMap.entries())
        ...
        .filter(renderData => renderData != null)
        .map(renderData => {
    ...
    const inlineDeclarationsRendered = await TemplateContainer.evaluate(
        ...
        {
            options: {
                quote: bundleOption.quote,
                useSemicolon: bundleOption.useSemicolon,
            },
            declarations: getInlineDeclarationRenderData(
                inlineDeclarations,
                bundleOption,
            ),
        },
    ...
    const exportsRendered = (
        await Promise.all(
            datas.map(async data => {
                const evaluated = await TemplateContainer.evaluate(
                    data.style,
                    data.renderData,
                );
                return evaluated;
    ...
    outputMap.set(
        output,
        [inlineDeclarationsRendered, ...exportsRendered].join("\n"),
    );
    const fileExistReason = await checkOutputFile(outputMap);
    if (
        !bundleOption.overwrite &&
        !bundleOption.backup &&
        fileExistReason.length > 0
    ) {
        Spinner.it.fail("ctix 'bundle' mode incomplete ...");
        Reasoner.it.start([
            ...fileExistReason,
            ...statementTable.getDuplicateReason(),
        ]);
        return;
    }
    const indexFiles = await Promise.all(
        Array.from(outputMap.entries())
            ...
            .map(async file => {
                return {
                    path: file.filePath,
                    content: await TemplateContainer.evaluate(
                        CE_TEMPLATE_NAME.INDEX_FILE_TEMPLATE,
                        {
                            directive: bundleOption.directive,
                            banner: getBanner(bundleOption, dayjs2()),
                            eol: extendOptions.eol,
    ...
    ProjectContainer.addSourceFilesAtPaths(
        bundleOption.project,
...
import { startSepAppend, startSepRemove as startSepRemove2 } from "my-node-fp";
import path19 from "path";
function getAllParentDir(parentDir, childDir) {
    const parent = startSepAppend(parentDir, path19.posix.sep);
    const child = startSepAppend(childDir, path19.posix.sep);
    const elements = startSepRemove2(
        child.replace(parent, ""),
        path19.posix.sep,
    )
        .split(path19.posix.sep)
        .slice(0, -1);
    return [
        parentDir,
...
async function dfsWalk(currentDirPath, callback) {
    ...
    const readed = await fs6.promises.readdir(resolved);
    const dirPaths = (
        await Promise.all(
...
import { replaceSepToPosix as replaceSepToPosix11 } from "my-node-fp";
import path20 from "path";
function getParentDir(rawDirPath) {
    const dirPath = replaceSepToPosix11(rawDirPath);
    const elements = dirPath.split(path20.posix.sep);
    ...
    return parentElements.join(path20.posix.sep);
}
...
import { getDirnameSync as getDirnameSync2 } from "my-node-fp";
async function creating(_buildOptions, createOption) {
    ...
    statementTable.inserts(statements);
    Spinner.it.start(
        `build ${`"${chalk5.green(createOption.exportFilename)}"`} file start`,
    );
    statements
        .filter(statement => !statementTable.isDuplicate(statement))
        .forEach(statement => {
    ...
    Array.from(filePathMap.entries())
        .map(([filePath, exportStatements]) =>
            getRenderData(createOption, filePath, exportStatements),
        )
        .filter(renderData => renderData != null)
        .map(renderData => {
            const comment = commentMap.get(renderData.filePath)?.comments.at(0);
        ...
        .forEach(data => {
            const key = getDirnameSync2(data.renderData.filePath);
    ...
    await dfsWalk(createOption.startFrom, params => {
        ...
        if (createOption.skipEmptyDir) {
            ...
            const parentDirs = getAllParentDir(
                createOption.startFrom,
                params.dirPath,
            );
            ...
            if (firstExistDir == null) {
                throw new Error(
                    "Cannot find the parent directory where the export statement is existed",
                );
            }
            const indexRednerData = createRenderData(
                ...
                {
                    importPath: addCurrentDirPrefix(
                        posixRelative(firstExistDir, params.dirPath),
                    ),
                    extname: {
                        origin: ".ts",
                        render: getImportStatementExtname(
                            createOption.fileExt,
                            ".ts",
                        ),
                    },
                    isHasDefault: false,
            ...
            const indexRednerData = createRenderData(
                ...
                {
                    importPath: addCurrentDirPrefix(
                        posixRelative(parentDir, params.dirPath),
                    ),
                    extname: {
                        origin: ".ts",
                        render: getImportStatementExtname(
    ...
    const rendereds = await Promise.all(
        Array.from(renderDataMap.entries()).map(async ([dirPath, datas]) => {
            const indexFilePath = posixJoin(
                dirPath,
                createOption.exportFilename,
            );
                ...
                return {
                    ...
                    content: await TemplateContainer.evaluate(
                        CE_TEMPLATE_NAME.INDEX_FILE_TEMPLATE,
                        {
                            ...
                            banner: getBanner(createOption, dayjs3()),
                            eol: extendOptions.eol,
                            content: file.fileContent,
                        },
                    ),
...
import { getDirname as getDirname5 } from "my-node-fp";
import path21 from "path";
async function getModuleRenderData(option, filePath, output) {
    const extname = getExtname(filePath);
    const renderExtname = extname;
    const filename = path21
        .basename(filePath)
        .replace(new RegExp(`${extname}$`), "");
    const dirPath = await getDirname5(filePath);
    const relativePath = addCurrentDirPrefix(
        posixRelative(await getDirname5(output), await getDirname5(filePath)),
    );
    const defaultExport = {
        path: {
            filename,
...
async function moduling(_buildOptions, moduleOption) {
    ...
    Spinner.it.update("include, exclude config");
    const output = posixResolve(
        posixJoin(moduleOption.output, moduleOption.exportFilename),
    );
    const include = new IncludeContainer({
        config: {
            include: getTsIncludeFiles({
    ...
    const datas = (
        await Promise.all(
            filenames.map(async filename => {
                const renderData = await getModuleRenderData(
                    moduleOption,
                    filename,
                    moduleOption.output,
                );
                ...
                return {
                    ...
                    content: await TemplateContainer.evaluate(
                        CE_TEMPLATE_NAME.INDEX_FILE_TEMPLATE,
                        {
                            ...
                            banner: getBanner(moduleOption, dayjs4()),
                            eol: extendOptions.eol,
                            content: file.fileContent,
                        },
                    ),
...
function getTsconfigComparer(cwd) {
    const comparer = (left, right) => {
        ...
        const leftBasename = path22.basename(left);
        const rightBasename = path22.basename(right);
...
import { exists as exists4 } from "find-up";
import { Glob as Glob3 } from "glob";
import inquirer from "inquirer";
import pathe7 from "pathe";
async function askInitOptions() {
    const cwd = process.cwd();
    const cwdAnswer = await inquirer.prompt([
        {
            type: "input",
            name: "cwd",
            default: cwd,
    ...
    const optionFilePath = pathe7.join(
        cwdAnswer.cwd,
        CE_CTIX_DEFAULT_VALUE.CONFIG_FILENAME,
    );
    const optionFileExist = await exists4(optionFilePath);
    const tsconfigGlob = new Glob3(["**/tsconfig.json", "**/tsconfig.*.json"], {
        cwd: cwdAnswer.cwd,
        ignore: defaultExclude,
    });
    const tsconfigFiles = getGlobFiles(tsconfigGlob);
    const packageJsonGlob = new Glob3(["**/package.json"], {
        cwd: cwdAnswer.cwd,
        ignore: defaultExclude,
    });
    const packageJsonFiles = getGlobFiles(packageJsonGlob);
    const sortedTsconfigFiles = tsconfigFiles.sort(
        getTsconfigComparer(cwdAnswer.cwd),
    );
    const sortedPackageJsonFiles = packageJsonFiles.sort(
        getTsconfigComparer(cwdAnswer.cwd),
    );
    const userSelectedAnswer = await inquirer.prompt([
        {
            type: "checkbox",
            name: "tsconfig",
        ...
        },
        {
            ...
            message:
                "Do you want to include all available options in the configuration file?",
            default: false,
        },
        {
            type: "list",
        ...
        },
        {
            ...
            message: answer2 =>
                `Do you want to create a backup file from ${answer2.configPosition}?`,
            default: true,
            when: answer2 => {
    ...
    if (userSelectedAnswer.configPosition === ".ctirc" && optionFileExist) {
        const overwriteAnswer = await inquirer.prompt([
            {
                type: "confirm",
                name: "overwirte",
                message: `Already exists ${chalk6.redBright(optionFilePath)}, overwrite it?`,
                default: false,
            },
        ]);
        if (!overwriteAnswer.overwirte) {
            return {
                ...
                packageJson: pathe7.join(
                    process.cwd(),
                    CE_CTIX_DEFAULT_VALUE.PACKAGE_JSON_FILENAME,
                ),
                mode: CE_CTIX_BUILD_MODE.BUNDLE_MODE,
...
async function getDefaultInitAnswer() {
    const cwd = process.cwd();
    const glob = new Glob4(["**/tsconfig.json", "**/tsconfig.*.json"], {
        cwd,
        ignore: defaultExclude,
    });
...
import { assign, parse as parse6, stringify } from "comment-json";
import fs7 from "fs";
import pathe9 from "pathe";
async function initializing(option) {
    await TemplateContainer.bootstrap();
    const answer = option.forceYes
        ? await getDefaultInitAnswer()
        : await askInitOptions();
    if (!answer.overwirte) {
        const optionFilePath = pathe9.join(
            answer.cwd,
            CE_CTIX_DEFAULT_VALUE.CONFIG_FILENAME,
        );
        Spinner.it.fail(`${chalk7.yellow(optionFilePath)} already exists`);
        return;
    }
    Spinner.it.start(
        `Start ctix ${chalk7.yellow("configuration")} generate ...`,
    );
    const nestedOptions = await Promise.all(
        answer.tsconfig.map(async tsconfigPath => {
            Spinner.it.update(
                `Start option creation for: ${chalk7.yellow(tsconfigPath)}`,
            );
            ...
            Spinner.it.succeed(
                `${chalk7.yellow(tsconfigPath)} option creation completed!`,
            );
            return {
                ...initOption,
    ...
    Spinner.it.start(`Start ${chalk7.yellow(".ctirc")} file write ...`);
    ...
    const parsedRenderedOptions = parse6(renderedOptions);
    if (answer.configPosition === "tsconfig.json") {
        await Promise.all(
            answer.tsconfig.map(async tsconfigFilePath => {
                const resolvedTsconfigFilePath =
                    pathe9.resolve(tsconfigFilePath);
                const buf = await fs7.promises.readFile(
                    resolvedTsconfigFilePath,
                );
                const parsedTsconfig = parse6(buf.toString());
                const newTsconfig = assign(parsedTsconfig, {
                    ctix: parsedRenderedOptions,
                });
                if (answer.confirmBackupPackageTsconfig) {
                    const backupFilePath = `${pathe9.basename(resolvedTsconfigFilePath)}.bak${pathe9.extname(resolvedTsconfigFilePath)}`;
                ...
                await fs7.promises.writeFile(
                    resolvedTsconfigFilePath,
                    stringify(newTsconfig, void 0, 2),
                );
                Spinner.it.succeed(
                    `${chalk7.yellow(resolvedTsconfigFilePath)} file modifing completed!`,
                );
            }),
        );
    } else if (answer.configPosition === "package.json") {
        const packageJsonFilePath = pathe9.resolve(answer.packageJson);
        const buf = await fs7.promises.readFile(packageJsonFilePath);
        const parsedPackageJson = parse6(buf.toString());
        const newPackageJson = assign(parsedPackageJson, {
            ctix: parsedRenderedOptions,
        });
        if (answer.confirmBackupPackageTsconfig) {
            const backupFilePath = `${pathe9.basename(packageJsonFilePath)}.bak${pathe9.extname(packageJsonFilePath)}`;
            await fs7.promises.writeFile(backupFilePath, buf.toString());
        }
        await fs7.promises.writeFile(
            packageJsonFilePath,
            stringify(newPackageJson, void 0, 2),
        );
        Spinner.it.succeed(
            `${chalk7.yellow(packageJsonFilePath)} file modifing completed!`,
        );
    } else {
        await fs7.promises.writeFile(
            ".ctirc",
            stringify(parsedRenderedOptions, void 0, 2),
        );
        Spinner.it.succeed(
            `${chalk7.yellow(".ctirc")} file writing completed!`,
        );
    }
}
...
function createRemoveOptions(argv) {
    const options = {
        ...
        exportFilename:
            argv.exportFilename ?? CE_CTIX_DEFAULT_VALUE.EXPORT_FILENAME,
        forceYes: argv.forceYes,
    };
    return options;
}
...
async function askRemoveFiles(filePaths) {
    //   inquirer2.registerPrompt("checkbox-plus", CheckboxPlusPrompt);
                ...
                return Promise.resolve(
                    fuse
                        ...
                        .filter(
                            matched =>
                                matched.percent >=
                                CE_CTIX_DEFAULT_VALUE.REMOVE_FILE_CHOICE_FUZZY,
                        )
...
import { getDirname as getDirname6 } from "my-node-fp";
async function getRemoveFileGlobPattern(argv, options) {
    const dirs = (
        await Promise.all(
            options.map(async modeOption => {
                const dir = await getDirname6(modeOption.project);
                const results = [
                    {
                        origin: dir,
...
import { isPromise as isPromise2 } from "util/types";
...
async function removing(options) {
    ...
    if (options.forceYes) {
        ...
        await filePaths.reduce(async (prevHandle, filePath) => {
            const handle = async () => {
                Spinner.it.succeed(
                    `${chalk8.redBright("removed:")} ${posixRelative(process.cwd(), filePath)}`,
                );
            };
            await prevHandle;
            return handle();
...
async function readJsonConfig(jsonConfigFilePath) {
    const buf = await fs8.promises.readFile(jsonConfigFilePath);
