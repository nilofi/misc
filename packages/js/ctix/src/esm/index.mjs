...
async function getOutputExcludedFiles(params) {
  ...
  const outputFiles = settify(outputDirPaths).map(
    (dirPath) => pathe.join(dirPath, params.exportFilename)
  );
  return outputFiles;
}
...
function getExportedKind(node) {
  ...
  return match(kind).with(tsm6.SyntaxKind.ClassDeclaration, () => {
    return {
...
import { getDirname as getDirname2, replaceSepToPosix as replaceSepToPosix3, startSepRemove } from "my-node-fp";
import path6 from "path";
import * as tsm7 from "ts-morph";
async function getExportStatement(sourceFile, option, extendOptions) {
  const dirPath = posixResolve(await getDirname2(sourceFile.getFilePath().toString()));
  const filename = startSepRemove(
    replaceSepToPosix3(sourceFile.getFilePath().toString().replace(dirPath, "")),
    path6.posix.sep
  );
  const relativePath = posixRelative(await getDirname2(option.project), dirPath);
      ...
      return symbols.statements.map((statement) => {
        if (statement.isDefault) {
          const reason2 = {
            ...
            filePath: pathe2.join(statement.path.dirPath, statement.path.filename),
...
import { getDirname as getDirname3, isDescendant, replaceSepToPosix as replaceSepToPosix5 } from "my-node-fp";
async function getExtendOptions(project) {
  ...
  const resolvedProjectDirPath = replaceSepToPosix5(await getDirname3(projectPath));
  const filePaths = tsconfig.fileNames.filter(
    (filePath) => isDescendant(resolvedProjectDirPath, filePath)
  );
  const topDirDepth = (await Promise.all(
    filePaths.map(async (filePath) => {
      const dirPath = replaceSepToPosix5(posixResolve(await getDirname3(filePath)));
      return {
        filePaths: [dirPath],
    ...
    (minDepth, depth) => {
      if (minDepth.depth > depth.depth) {
        return { ...depth, filePaths: settify2(minDepth.filePaths.concat(depth.filePaths)) };
      }
      if (minDepth.depth === depth.depth) {
        return { ...minDepth, filePaths: settify2(minDepth.filePaths.concat(depth.filePaths)) };
      }
      return minDepth;
    },
    {
...
async function readConfigFromFile(configFilePath) {
  try {
    ...
    if (typeof parsed !== "object") {
      return fail4(new Error(`invalid configuration file format: ${parsed}`));
    }
    return pass4(parsed);
  } catch (caught) {
    const err = isError4(caught, new Error("unknown error raised from configuration reading"));
    return fail4(err);
  }
}
...
async function readConfigFromPackageJson() {
  try {
    ...
    const err = isError5(caught, new Error("unknown error raised from configuration reading"));
    return fail5(err);
  }
}
...
async function readConfigFromTsconfigJson(tsconfigFilePath) {
  try {
    ...
    return fail6(new Error(`cannot read configuration from ${tsconfigFilePath}`));
  } catch (caught) {
    const err = isError6(caught, new Error("unknown error raised from configuration reading"));
    return fail6(err);
  }
}
...
var _ProgressBar = class _ProgressBar {
  ...
  set stream(value) {
    if (value !== __privateGet(this, _stream)) {
      ...
      __privateSet(this, _bar, new SingleBar({
...
var _Reasoner = class _Reasoner {
  ...
  static messaging(reason) {
    const messageBlock = [""];
    const typeMessage = reason.type === "error" ? chalk3.bgRed(`   ${reason.type.toUpperCase()}   `) : chalk3.bgYellow(`   ${chalk3.black(reason.type.toUpperCase())}    `);
    ...
    const chevronRight = reason.type === "error" ? chalk3.red(">") : chalk3.yellow(">");
    messageBlock.push(`${typeMessage} ${filename}`);
    if (reason.lineAndCharacter == null) {
      messageBlock.push(`   ${chevronRight} ${chalk3.gray(`${filePath}`)}`);
    } else {
      messageBlock.push(
...
var _Spinner = class _Spinner {
  ...
  set stream(value) {
    if (value !== __privateGet(this, _stream3)) {
      ...
      __privateSet(this, _spinner, ora({ text: __privateGet(this, _spinner).text, stream: process[value] }));
...
async function transformCreateMode(argv, option) {
  const startFrom = argv.startFrom ?? option.startFrom ?? pathe5.resolve(await getDirname4(argv.project));
  const resolvedStartFrom = pathe5.isAbsolute(startFrom) ? startFrom : pathe5.resolve(startFrom);
  return {
...
function getTsIncludeFiles(config) {
  ...
  const filePaths = config.extend.tsconfig.fileNames.filter(
    (filePath) => isDescendant2(config.extend.resolved.projectDirPath, filePath)
  );
  return filePaths;
}
...
async function createBuildOptions(argv) {
  ...
  const tsconfig = getTypeScriptConfig(projectPath);
  const include = argv.include != null ? toArray2(argv.include) : getTsIncludeFiles({
    config: { include: [] },
    extend: { tsconfig, resolved: { projectDirPath: projectPath } }
  });
  const exclude = argv.exclude != null ? toArray2(argv.exclude) : getTsExcludeFiles({
    config: { exclude: [] },
    extend: { tsconfig }
  });
...
function getGlobFiles(glob) {
  ...
  for (const filePath of glob) {
    filePathSet.add(
      typeof filePath === "string" ? replaceSepToPosix7(filePath) : replaceSepToPosix7(path13.join(filePath.path, filePath.name))
    );
  }
...
var ExcludeContainer = class {
  constructor(params) {
    ...
    params.inlineExcludeds.forEach((inlineExcluded) => {
      const filePath = path14.isAbsolute(inlineExcluded.filePath) ? replaceSepToPosix8(inlineExcluded.filePath) : posixResolve(inlineExcluded.filePath);
...
var _TemplateContainer = class _TemplateContainer {
  constructor(args) {
    ...
    __privateSet(this, _eta, new Eta({ views: "ctix", autoEscape: false, rmWhitespace: true }));
  ...
  static async readFiles(basePath) {
    ...
    const buffers = await Promise.all([
      ...
      fs5.promises.readFile(n(CE_TEMPLATE_NAME.MODULE_INDEX_FILE_TEMPLATE)),
      fs5.promises.readFile(n(CE_TEMPLATE_NAME.DECLARATION_FILE_TEMPLATE)),
      fs5.promises.readFile(n(CE_GENERATION_STYLE.DEFAULT_ALIAS_NAMED_STAR)),
      fs5.promises.readFile(n(CE_GENERATION_STYLE.DEFAULT_ALIAS_NAMED_DESTRUCTIVE)),
      fs5.promises.readFile(n(CE_GENERATION_STYLE.DEFAULT_NON_ALIAS_NAMED_DESTRUCTIVE)),
      fs5.promises.readFile(n(CE_GENERATION_STYLE.DEFAULT_STAR_NAMED_STAR)),
      fs5.promises.readFile(n(CE_GENERATION_STYLE.DEFAULT_STAR_NAMED_DESTRUCTIVE))
    ]);
    const [
      indexFile,
      options,
  ...
  static async evaluate(name, data, option) {
    try {
      ...
      const err = isError8(caught, new Error("raise error from evaluateTemplate"));
...
function getInlineDeclarationRenderData(declarations, options) {
  const renderDatas = declarations.map((declaration) => {
    ...
    const importPath = options.output != null ? addCurrentDirPrefix(posixRelative(options.output, pathe6.join(dirname, basename2))) : replaceSepToPosix9(`.${path17.posix.sep}${pathe6.join(dirname, basename2)}`);
    return {
      ...declaration,
      importPath,
      extname: {
...
import { basenames, replaceSepToPosix as replaceSepToPosix10 } from "my-node-fp";
...
function getRenderData(option, filePath, statements, output) {
  ...
  const relativePath = output != null ? addCurrentDirPrefix(posixRelative(output, filename)) : replaceSepToPosix10(`.${path18.posix.sep}${basenames(filename, getExtname(filePath))}`);
  return {
    options: {
      quote: option.quote,
...
async function bundling(buildOptions, bundleOption) {
  ...
  Spinner.it.start(`build ${`"${chalk4.green(bundleOption.exportFilename)}"`} file start`);
...
function getAllParentDir(parentDir, childDir) {
  ...
  const elements = startSepRemove2(child.replace(parent, ""), path19.posix.sep).split(path19.posix.sep).slice(0, -1);
  return [
    parentDir,
...
async function creating(_buildOptions, createOption) {
  ...
  const statementTable = new StatementTable();
  statementTable.inserts(statements);
  Spinner.it.start(`build ${`"${chalk5.green(createOption.exportFilename)}"`} file start`);
  statements.filter((statement) => !statementTable.isDuplicate(statement)).forEach((statement) => {
    const filePath = posixJoin(statement.path.dirPath, statement.path.filename);
...
async function getModuleRenderData(option, filePath, output) {
  ...
  const filename = path21.basename(filePath).replace(new RegExp(`${extname}$`), "");
...
async function askInitOptions() {
  ...
  const optionFilePath = pathe7.join(cwdAnswer.cwd, CE_CTIX_DEFAULT_VALUE.CONFIG_FILENAME);
  ...
  if (userSelectedAnswer.configPosition === ".ctirc" && optionFileExist) {
    ...
    if (!overwriteAnswer.overwirte) {
      return {
        ...
        packageJson: pathe7.join(process.cwd(), CE_CTIX_DEFAULT_VALUE.PACKAGE_JSON_FILENAME),
        mode: CE_CTIX_BUILD_MODE.BUNDLE_MODE,
...
async function getDefaultInitAnswer() {
  ...
  const answer = {
    ...
    packageJson: pathe8.join(process.cwd(), CE_CTIX_DEFAULT_VALUE.PACKAGE_JSON_FILENAME),
    mode: CE_CTIX_BUILD_MODE.BUNDLE_MODE,
    exportFilename: CE_CTIX_DEFAULT_VALUE.EXPORT_FILENAME,
    addEveryOptions: false,
    configComment: true,
...
async function initializing(option) {
  ...
  if (!answer.overwirte) {
    const optionFilePath = pathe9.join(answer.cwd, CE_CTIX_DEFAULT_VALUE.CONFIG_FILENAME);
    Spinner.it.fail(`${chalk7.yellow(optionFilePath)} already exists`);
    return;
  }
  Spinner.it.start(`Start ctix ${chalk7.yellow("configuration")} generate ...`);
  const nestedOptions = await Promise.all(
    answer.tsconfig.map(async (tsconfigPath) => {
      Spinner.it.update(`Start option creation for: ${chalk7.yellow(tsconfigPath)}`);
      ...
      Spinner.it.succeed(`${chalk7.yellow(tsconfigPath)} option creation completed!`);
      return {
        ...initOption,
  ...
  if (answer.configPosition === "tsconfig.json") {
    await Promise.all(
      answer.tsconfig.map(async (tsconfigFilePath) => {
        const resolvedTsconfigFilePath = pathe9.resolve(tsconfigFilePath);
        const buf = await fs7.promises.readFile(resolvedTsconfigFilePath);
        ...
        await fs7.promises.writeFile(resolvedTsconfigFilePath, stringify(newTsconfig, void 0, 2));
        Spinner.it.succeed(`${chalk7.yellow(resolvedTsconfigFilePath)} file modifing completed!`);
      })
    );
    ...
    await fs7.promises.writeFile(packageJsonFilePath, stringify(newPackageJson, void 0, 2));
    Spinner.it.succeed(`${chalk7.yellow(packageJsonFilePath)} file modifing completed!`);
  } else {
    await fs7.promises.writeFile(".ctirc", stringify(parsedRenderedOptions, void 0, 2));
    Spinner.it.succeed(`${chalk7.yellow(".ctirc")} file writing completed!`);
  }
}
...
function getRatioNumber(num, base = 1) {
  return bignumber(1).sub(bignumber(num)).mul(100 * base).floor().div(100).toNumber();
}
...
function endsEol(content, eol, size) {
  ...
  const eols = populate2(multiplier).map(() => eol).join("");
  return `${content}${eols}`;
}
